...
class SellListViewHolder(private val binding: ListItemBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(sell: ProductsList) = with(binding.root) {
        ...
        binding.txtItemPriceValue.text = sell.price.toString().replace('.', ',')
    }
}
